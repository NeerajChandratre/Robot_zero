...
while True:
    ...
    if lbls[0] == [] or lbls[1] == []:
        ...
        print('right_img.shape[0] is ')
        print(right_img.shape[0])
        print('right_img.shape[1] is ')
        print(right_img.shape[1])
        #disparity_of_two_cmras = pnts1 = np.array(top_left_box_values(det[0]))[:,0] - np.array(bottom_right_box_values(det[1]))[:,0]
        get_cntr_of_bxes_left = get_center_of_boxes(det[0])[0]
        u_l = get_cntr_of_bxes_left[0]
        v_l = get_cntr_of_bxes_left[1]
        print('u_l is ',u_l)
        print('v_l is ',v_l)
        get_cntr_of_bxes_rght = get_center_of_boxes(det[1])[0]
        u_r = get_cntr_of_bxes_rght[0]
        v_r = get_cntr_of_bxes_rght[1]
        o_x = left_img.shape[1]/2
        o_y = left_img.shape[0]/2
        print('o_x is ',o_x)
        print('o_y is ',o_y)
        disparity = u_l - u_r
        b = 5.5
        x_co_ord = (b*(u_l - o_x))/disparity  #taken frm Frst prncpls of Comp Vision, Colmbia Univrsty.
        y_co_ord = (b*(v_l - o_y))/disparity
        print('x_co_ord is ',x_co_ord)
        print('y_co_ord is ',y_co_ord)
...
# above code works well for getting x co-ordint and depth although improvements are needed in both.
